...
internal class UnsafeListTests : CollectionsTestCommonBase
{
    ...
    public unsafe void UnsafeListT_AddNoResize()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => { fixed (int* r = range) container.AddRangeNoResize(new ReadOnlySpan<int>(r, 17) ); });
#endif

        container.SetCapacity(17);
        Assert.DoesNotThrow(() => { fixed (int* r = range) container.AddRangeNoResize(r, 17); });
        container.Clear();
        Assert.DoesNotThrow(() => { fixed (int* r = range) container.AddRangeNoResize(new ReadOnlySpan<int>(r, 17)); });
    ...
    public unsafe void UnsafeListT_RemoveAtSwapBack()
    {
        ...
        fixed (int* r = range) list.AddRange(new ReadOnlySpan<int>(r, 10));
